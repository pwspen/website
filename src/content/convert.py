...
converter_agent = Agent(
    ...
    system_prompt=f'Convert the given HTML blog post (post name also given) to a MDX format. Here is an example of the MDX format: {target_example}'
                   ...
                   'Note that the mdx hero image is the thumbnail *and also* automatically inserted at the top of the post, so the example will actually has the same image repeated twice. Do not replicate this - images should be either the hero or in the body, not both.'
                   'If there is an image at the start of the post, make it the hero image, and *do not* put it in an <image> component. Otherwise, notify the user with your notify_missing_hero tool, then continue.'
...
def notify_missing_hero(postname: str) -> None:
    logging.info(f'Could not find the hero image for the post "{postname}"')

@converter_agent.tool_plain
def notify_extra_attn(postname: str, reason: str) -> None:
    logging.info(f'{postname} needs extra attention: {reason}')

results_folder = 'src/content/blog'
def save_mdx(postname: str, mdx: str) -> None:
    with open(f'{results_folder}/{postname}.mdx', 'w') as f:
        f.write(mdx)

if __name__ == '__main__':
    fnames: list[str] = glob.glob('src/content/old_posts_html/*.html')
    print(fnames)
    for fname in fnames:
